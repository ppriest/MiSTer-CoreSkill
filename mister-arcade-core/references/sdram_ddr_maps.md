...
|---|---|---|
| the Psikyo core's `docs/phase1_memory_map.md` | 68EC020 base map (:30-48) + per-board overlays sngkace/gunbird (:50-75); Z80/LZ8420M program + I/O maps (:77-116); ADPCM-A bit-swap fixup and where it lives in RTL (:118-174); sprite RAM / VRAM / vreg layouts (:176-265) | `Range \| Size \| Contents \| Access` (base map); `Range \| Contents` (overlays); `Port \| Contents` (I/O); `Offset (bytes) \| Contents` (vregs). Every row cites `psikyo.cpp:` line |
| the Psikyo core's `docs/phase1_ddram_map.md` | SUPERSEDED (header :3-12). DDRAM protocol (:22-62), address map (:82-108), arbiter consumer list (:124-141), throughput failure (:143-167) | `Region \| Base (offset from 0x30000000) \| Size \| Rounded up from` (:93-101) |
| the Psikyo core's `docs/phase1_sdram_map.md` | Why SDRAM (:8-56); SDRAM_* interface + controller semantics (:58-95); 64-bit granule problem (:97-121); address map (:123-144); port partition, original and current (:146-175); measured verification (:177-231); narrow-bridge, byte-order bug and adapter (:233-301) | Map: `Region \| Base \| Size` (:129-137). Ports: `Physical port \| Logical consumers \| Arbitration` (:155-159, :166-170) |
| the Psikyo core's `docs/phase2_sh404.md:117-129` | Re-layout table after Tengai's 4 MB tiles | `Region \| Base \| Size` (third copy of the same map) |
| the Psikyo core's `docs/savestates.md:49-57` | DDR3 collision list: rotator `0x24000000` 3x8 MB, loader window, proposed `0x3E000000` | prose bullets |
...
| Where | Content | Form |
|---|---|---|
| the Fuuki core's `docs/ROADMAP.md:257-282` | Main CPU map, FG-2/FG-3 differences inline | `Address \| Size \| Contents` |
...
| the Fuuki core's `docs/gfx_layouts.md:12-19`, 96-113` | Region -> gfx layout; byte order measured (`_SWAP` wins on all five ROMs) | `Region \| FG-2 \| FG-3`; `ROM \| Region \| swapped \| not swapped` |
| the Fuuki core's `rtl/memory/fuuki_sdram_top.sv:1-19` | Port assignment with deadline rationale and an unmeasured bandwidth estimate; "This module is the authority" | header comment |

### Seta (no map doc; per-game CPU map lives in RTL)

| Where | Content | Form |
|---|---|---|
| the Seta core's `docs/ROADMAP.md:631-660` | Group C/D 68000 map only; notes Group A/B "use the same regions at different base addresses" | `range \| contents` |
...
| the Seta core's `docs/LESSONS_LEARNED.md:380-387` | Byte order fixed at the seam with an adapter | lesson |
| the Seta core's `rtl/memory/seta_sdram_top.sv:1-19` | Ports; seven layouts A-G in one-line prose each | header comment |

### JalecoMS32 (HARDWARE_NOTES + ROADMAP + RTL header)

| Where | Content | Form |
|---|---|---|
| the JalecoMS32 core's `docs/ROADMAP.md:202-224` | V70 map | `region \| window \| physical size \| width` -- the only core whose CPU map records bus width |
| the JalecoMS32 core's `docs/HARDWARE_NOTES.md:31-52` | V70 map vs Charles MacDonald's board measurements: mirrors, widths, unmapped reads | `Note \| Core \| Status` (matches/changed/differs) |
| `...\ROADMAP.md:520-538` | Memory plan: which region on SDRAM / DDR3 / BRAM and why | `what \| where \| why` |
| the JalecoMS32 core's `docs/phase1_video.md:21-24`, 76-77` | DDRAM traffic and latency numbers; SDRAM bytes per tile row | table rows |
| the JalecoMS32 core's `rtl/memory/ms32_sdram_top.sv:6-24` | SDRAM map as a comment table: name, base, size, contents, interleave, decrypt-on-the-way-in | header comment |
...
| Where | Content | Form |
|---|---|---|
| the KonamiGX core's `docs/ROADMAP.md:538-566` | 68EC020 Type 2 map | `region \| window \| size \| notes` |
...
| the KonamiGX core's `rtl/memory/gx_sdram_top.sv:1-32` | Ports; image layout in prose; the 5->8 spread formula | header comment |
| the KonamiGX core's `rtl/gx_board_cfg.sv:5-12` | "The SDRAM image ... bases and four-byte part sizes are what build_mra.py derives" | header comment |
| the KonamiGX core's `docs/WORKFLOW.md:345-358` | Rule: "SDRAM offsets come from the RTL's own address map, never duplicated into the generator" | rule list |

DDRAM: the KonamiGX core's `KonamiGX.sv:32` ties every `DDRAM_*` output to 0. No DDRAM use, no
...
|---|---|---|---|---|
| Psikyo | the Psikyo core's `rtl/memory/psikyo_sdram_top.sv:144-154` | `localparam logic [24:0] <REGION>_BASE = 25'h...` (MAINCPU, AUDIOCPU, SPRITES, TILES, SPRITELUT, ADPCMA) | 25-bit byte, 32 MB | comment :137-143 gives the sizing rule and "they and this table must change together" |
| Fuuki | the Fuuki core's `rtl/memory/fuuki_sdram_top.sv:99-107` (FG2), `:109-120` (FG3), `:122-129` board mux | `localparam logic [25:0] FG{2,3}_BASE_<REGION> = 26'h...;   // <size>` | 26-bit byte, 64 MB | trailing comment = reserved size; :110-112 hole rationale; `BOARD_FG2/FG3` = mod byte bit 0 |
| Seta | the Seta core's `rtl/memory/seta_sdram_top.sv:105-154` | `BASE_<REGION>_<LAYOUT>` per layout A-G, then wires `BASE_GFX1..X1SND` muxed by `layout` (:131-150), `SIZE_GFX1` (:159-164) | 26-bit | trailing comment = size; :152 "localparams, not wires, so build_mra.py can read them"; layout index from `seta_board_cfg.sv:83-84` |
| MS32 | the JalecoMS32 core's `rtl/memory/ms32_sdram_top.sv:103-113` | `BASE_<REGION>` + `MASK_TX/BG/ROZ` (engines mask to region; `.mra` repeats ROM to region size) | 26-bit | sizes only in the header table :13-20 and in `build_mra.py` `MAP` |
| KonamiGX | the KonamiGX core's `rtl/gx_board_cfg.sv:34-45` | `case(game)` arms: `tile_base, obj_base` (26-bit byte) + `tile_size4, obj_size4` (bytes in the four-byte part); `gx_sdram_top.sv:85` only `BASE_MAINCPU` | 26-bit | arms are GENERATED (`build_mra.py --cfg`, markers :34/:45); per set, not per board |
...
- Seta: the Seta core's `rtl/cpu/maincpu.sv:13-44` `board_t` (21 maps) and `:231-262` default base table + `case(board)` overrides; comment `:14` "Must agree with scripts/mame_capture.py's FAMILIES table" (agreement not machine-checked -- unverified). Selected by `seta_board_cfg.sv` `map_board`.
...
| MS32 | sprite frame buffer, 2 banks x 0x40000 at window byte `0x2000000`; pixel formula | the JalecoMS32 core's `rtl/video/ms32_sprite_fb.sv:32-39` |
| MS32 | object RAM copy at window byte `0x2100000` | the JalecoMS32 core's `rtl/video/ms32_objram.sv:13-17`, 39` |
| MS32 | one-port sharing (core burst jobs vs rotator FIFO) | the JalecoMS32 core's `rtl/memory/ms32_ddram_mux.sv:1-20` |
...
|---|---|---|---|
| Seta | `read_sdram_map()` regex-parses every `localparam ... BASE_\w+` from the RTL; `sys.exit` if any layout's name is missing | the Seta core's `scripts/build_mra.py:177-190`, LAYOUTS `:78-105`, docstring `:36-38` | `.mra` offsets == RTL; whole image re-read via `mra.py` and compared (`:20-29`); clone region sizes == parent (`:216-240`) |
| Fuuki | same regex on `FG[23]_BASE_\w+`; exits if missing; prints the map | the Fuuki core's `scripts/build_mra.py:19-20`, 408-431, 697-702` | `.mra` offsets == RTL; byte-for-byte re-read (`:7-16`) |
| MS32 | `MAP` list REPEATS bases+sizes; `check_map()` asserts each `BASE_<NAME>` in the RTL equals the list; region start position must equal map base | the JalecoMS32 core's `scripts/build_mra.py:48-58`, 192-195` | duplicated table cannot drift silently; sizes are NOT checked against RTL (RTL has no size localparams) |
| KonamiGX | direction reversed: `layout()` derives bases from `ROM_START`; `rtl_arm()` parses the generated case arm; build refuses if they differ; `--cfg` prints arms to paste | the KonamiGX core's `scripts/build_mra.py:100-142`, 293-298` | RTL arm == ROM_START-derived layout |
| KonamiGX | sim: stream the `.mra` image into `gx_sdram_top` + chip model, read every region back via the board's port, compare to MAME region images | the KonamiGX core's `scripts/check_gx_sdram.py:1-24` | download transform + byte order end to end |
| Seta | hardware: JTAG probe of the last gfx2 granule vs `build_region.py` image; single-granule check | the Seta core's `scripts/sdram_check.py:1-30`, `check_granule.py:1-24` | SDRAM content at address, on the board |
...
| Psikyo | none for the map. `validate_mra.py` checks XML well-formedness, stray text, `<switches>` byte count, `<setname>`, `<rotation>` only | the Psikyo core's `scripts/validate_mra.py:19-33` | not the offsets |
| Psikyo | `verify_rom_trace.py`: hardware ROM-read trace vs zip, brute-forces interleaves | described at the Seta core's `docs/LESSONS_LEARNED.md:192-198` | content at traced addresses, not reach |
