...
--   CORE_IO_LO/HI                       I/O and video range whose READS are logged
...
local IO_LO, IO_HI   = hex("CORE_IO_LO", "0"), hex("CORE_IO_HI", "0")
...
f:write(string.format("# main CPU: all writes, reads of %X-%X (I/O), %X-%X (vectors)\n",
                      IO_LO, IO_HI, VEC_LO, VEC_HI))
...
if IO_HI > IO_LO then
    core_subs[#core_subs + 1] = prog:install_read_tap(IO_LO, IO_HI, "core_sys_r", guard("r"))
end
